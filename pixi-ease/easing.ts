...
export class Easing extends Events {
     ...
     */
    constructor(element: Container | Container[], params: object, options: object) {
        super()
    ...
    addParam(element: Container, entry: string, param: any) {
        ...
        switch (entry) {
            ...
            case 'blend':
                ...
                to = colors.length
                delta = to
                update = (entry === 'tint') ? (ease: Ease) => this.updateTint(ease, colors) : (ease: Ease) => this.updateBlend(ease, colors)
                break
