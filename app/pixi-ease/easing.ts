...
export class Easing extends Events {
  ...
  addParam(element: Container, entry: string, param: any) {
    let start,
      to,
      delta,
      update,
   ...
   * @param {Container} [element] if not set, removes all elements in this easing
   * @param {(string|string[])} [params] if not set, removes all params for each element
   */
  remove(element?: Container, params?: string | string[]): boolean | undefined {
    if (arguments.length === 0) {
      ...
      for (let i = 0; i < this.eases.length; i++) {
        ...
        if (
          (!element || ease.element === element) &&
          (!params || params.indexOf(ease.entry) !== -1)
        ) {
  ...
  updateOne(ease: any, entry: string) {
    ease.element[entry] = this.options.ease(
      this.time,
      ease.start,
      ease.delta,
  ...
  updatePoint(ease: any, entry: string) {
    ease.element[entry].x = ease.element[entry].y = this.options.ease(
      this.time,
      ease.start,
      ease.delta,
      this.options.duration,
  ...
  updateCoord(ease: any, name: string, coord: string) {
    ease.element[name][coord] = this.options.ease(
      this.time,
      ease.start,
      ease.delta,
      this.options.duration,
  ...
  update(elapsed: number): boolean | undefined {
    if (this.eases.length === 0) {
      return true
    }
